...
LOCAL_PATH=$(pwd)
bass_patch_dir="${LOCAL_PATH}/patches-bassos-v"
...
cd .repo/manifests
rm default.xml
cp ${LOCAL_PATH}/manifests/blissos-v-static.xml bliss.xml
cp ${LOCAL_PATH}/manifests/bass15.xml bass.xml
...
# copy vendor configs
if [ -f ${LOCAL_PATH}/configs/vendor/bliss_os_v/vendor_config.cfg ]; then
    cp ${LOCAL_PATH}/configs/vendor/bliss_os_v/vendor_config.cfg .config/vendor_config.cfg
fi
