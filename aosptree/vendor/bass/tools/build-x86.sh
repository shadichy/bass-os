...
export USE_PDFVIEWER_PLATFORM=false
export USE_PRINT_PLATFORM=false
export USE_BVNC_PLATFORM=false
export USE_FDEX11_PLATFORM=false
export BLISS_2BTN_NAVIGATION=false
export SKIP_AAROPA_DOWNLOAD=false
export BOARD_IS_SURFACE_BUILD=false
export BOARD_IS_ZENITH_BUILD=false
export BOARD_IS_STABLE_BUILD=false
export PER_DISPLAY_FOCUS_ENABLED=false
export NO_BASS_BRANDING_INCLUDED=false
export WITH_ADB_INSECURE=false
export ADDON_WALLPAPER=false
export USE_SQUASHFS=false
export USE_EROFS=false
export USE_BLISS_BOOT_CONFIG=false
export ANDROID_USE_INTEL_HOUDINI=false
export USE_WIDEVINE=false
export SET_DARKMODE=false
export USE_POINTER_CHOREOGRAPHER=false
export USE_SSHD4A_PLATFORM=false
...
function displayHelp() {
    ...
    echo "--gpdf          Include minimal PDF Viewer"
    echo "--printp          Include minimal print service"
    echo "--bvnc          Include bVNC app"
    echo "--fdex11          Include OpenFDE-x11"
    echo "--2btnnav          Build with 2-button navigation set as default"
    echo "--skipad          Skip Aaropa Download"
    echo "--mskernel          Build with surface kernel"
    echo "--zkernel          Build with zenith kernel"
    echo "--skernel          Build with stable kernel"
    echo "--pdfe          enable config_perDisplayFocusEnabled be default in build"
    echo "--androidify          Do not include branding assets for bootanimation and wallpaper"
    echo "--adbi          use insecure adb by default"
    echo "--wpaddon          Include wallpaper changer addon"
    echo "--sfs          use squashfs for system image"
    echo "--efs          use erofs for system image"
    echo "--bbo          Include Bliss Boot Options app"
    echo "--houdini          Include houdini (REQUIRES OUTSIDE SOURCES)"
    echo "--widevine          Include widevine (REQUIRES OUTSIDE SOURCES)"
    echo "--darkmode          Set dark theme as default on build"
    echo "--usepointerc          Use pointer choreographer"
    echo "--sshd4a          Include SSHD control app"
    ...
    echo "--pclauncher          Include experimental PC Launcher from Google's Desktop AVD"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --gpdf)
               USE_PDFVIEWER_PLATFORM=true
               shift
               ;;
        --printp)
               USE_PRINT_PLATFORM=true
               shift
               ;;
        --bvnc)
               USE_BVNC_PLATFORM=true
               shift
               ;;
        --fdex11)
               USE_FDEX11_PLATFORM=true
               shift
               ;;
        --2btnnav)
               BLISS_2BTN_NAVIGATION=true
               shift
               ;;
        --skipad)
               SKIP_AAROPA_DOWNLOAD=true
               shift
               ;;
        --mskernel)
               BOARD_IS_SURFACE_BUILD=true
               shift
               ;;
        --zkernel)
               ...
               ;;
        --skernel)
               BOARD_IS_STABLE_BUILD=true
               shift
               ;;
        --pdfe)
               PER_DISPLAY_FOCUS_ENABLED=true
               shift
               ;;
        --androidify)
               NO_BASS_BRANDING_INCLUDED=true
               shift
               ;;
        --adbi)
        ...
        --darkmode)
               ...
               ;;
        --usepointerc)
               USE_POINTER_CHOREOGRAPHER=true
               shift
               ;;
        --sshd4a)
               USE_SSHD4A_PLATFORM=true
               shift
               ;;
        --mdsp)
...
export USE_PDFVIEWER_PLATFORM=${USE_PDFVIEWER_PLATFORM:-false};
export USE_PRINT_PLATFORM=${USE_PRINT_PLATFORM:-false};
export USE_BVNC_PLATFORM=${USE_BVNC_PLATFORM:-false};
export USE_FDEX11_PLATFORM=${USE_FDEX11_PLATFORM:-false};
export BLISS_2BTN_NAVIGATION=${BLISS_2BTN_NAVIGATION:-false};
export SKIP_AAROPA_DOWNLOAD=${SKIP_AAROPA_DOWNLOAD:-false};
export BOARD_IS_SURFACE_BUILD=${BOARD_IS_SURFACE_BUILD:-false};
export BOARD_IS_ZENITH_BUILD=${BOARD_IS_ZENITH_BUILD:-false};
export BOARD_IS_STABLE_BUILD=${BOARD_IS_STABLE_BUILD:-false};
export PER_DISPLAY_FOCUS_ENABLED=${PER_DISPLAY_FOCUS_ENABLED:-false};
export NO_BASS_BRANDING_INCLUDED=${NO_BASS_BRANDING_INCLUDED:-false};
export WITH_ADB_INSECURE=${WITH_ADB_INSECURE:-false};
export ADDON_WALLPAPER=${ADDON_WALLPAPER:-false};
export USE_SQUASHFS=${USE_SQUASHFS:-false};
export USE_EROFS=${USE_EROFS:-false};
export USE_BLISS_BOOT_CONFIG=${USE_BLISS_BOOT_CONFIG:-false};
export ANDROID_USE_INTEL_HOUDINI=${ANDROID_USE_INTEL_HOUDINI:-false};
export USE_WIDEVINE=${USE_WIDEVINE:-false};
export SET_DARKMODE=${SET_DARKMODE:-false};
export USE_POINTER_CHOREOGRAPHER=${USE_POINTER_CHOREOGRAPHER:-false};
export USE_SSHD4A_PLATFORM=${USE_SSHD4A_PLATFORM:-false};
...
export USE_PCLAUNCHER=${USE_PCLAUNCHER:-false};
...
echo "IncludeminimalPDFViewer: ${USE_PDFVIEWER_PLATFORM}";
echo "Includeminimalprintservice: ${USE_PRINT_PLATFORM}";
echo "IncludebVNCapp: ${USE_BVNC_PLATFORM}";
echo "IncludeOpenFDEx11: ${USE_FDEX11_PLATFORM}";
echo "Buildwith2buttonnavigationsetasdefault: ${BLISS_2BTN_NAVIGATION}";
echo "SkipAaropaDownload: ${SKIP_AAROPA_DOWNLOAD}";
echo "Buildwithsurfacekernel: ${BOARD_IS_SURFACE_BUILD}";
echo "Buildwithzenithkernel: ${BOARD_IS_ZENITH_BUILD}";
echo "Buildwithstablekernel: ${BOARD_IS_STABLE_BUILD}";
echo "EnableconfigperDisplayFocusEnabledbedefaultinbuild: ${PER_DISPLAY_FOCUS_ENABLED}";
echo "Donotincludebrandingassetsforbootanimationandwallpaper: ${NO_BASS_BRANDING_INCLUDED}";
echo "Useinsecureadbbydefault: ${WITH_ADB_INSECURE}";
echo "Includewallpaperchangeraddon: ${ADDON_WALLPAPER}";
echo "Usesquashfsforsystemimage: ${USE_SQUASHFS}";
echo "Useerofsforsystemimage: ${USE_EROFS}";
echo "IncludeBlissBootOptionsapp: ${USE_BLISS_BOOT_CONFIG}";
echo "IncludehoudiniREQUIRESOUTSIDESOURCES: ${ANDROID_USE_INTEL_HOUDINI}";
echo "IncludewidevineREQUIRESOUTSIDESOURCES: ${USE_WIDEVINE}";
echo "Setdarkthemeasdefaultonbuild: ${SET_DARKMODE}";
echo "Usepointerchoreographer: ${USE_POINTER_CHOREOGRAPHER}";
echo "IncludeSSHDcontrolapp: ${USE_SSHD4A_PLATFORM}";
...
echo "IncludeexperimentalPCLauncherfromGooglesDesktopAVD: ${USE_PCLAUNCHER}";
...
if [ "$PER_DISPLAY_FOCUS_ENABLED" = true ]; then
    # set 'config_perDisplayFocusEnabled">false' to 'config_perDisplayFocusEnabled">true'
    # in device/generic/common/overlay/frameworks/base/core/res/res/values/config.xml
    ...
    # set 'config_perDisplayFocusEnabled">true' to 'config_perDisplayFocusEnabled">false'
    # in device/generic/common/overlay/frameworks/base/core/res/res/values/config.xml
...
if [ "$BUILD_ONLY_EXTRA_PACKAGES" = true ]; then
    make ${BUILD_EXTRA_PACKAGES} -j${cores_1_8};
else
