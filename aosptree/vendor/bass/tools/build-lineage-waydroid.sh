...
build_filename="Bass_Waydroid_${build_date}"
...
export BASS_BUILD_VARIANT=vanilla;
export LINEAGE_BUILDTYPE=vanilla;
...
function displayHelp() {
    ...
    echo "--nobassinit          Don't use Bass init in build"
    echo "--vapor          Use Vapor Launcher as default home"
    echo "--darkmode          Set dark theme as default on build"
    echo "--arch          Default build arch (arm, arm64, x86, x86_64)"
...
doImageCopy() {
    ...
    # Look in out/target/product/*/ for the .img, .sha256 and Changelog* files,
    # and copy them to a new /images directory. using the filename of the .img for the directory name and the .img file name.
    images_exists=$(find out/target/product/waydroid_${BASS_ARCH}/ -maxdepth 1 -mindepth 1 -type f -name "*.img")
    if [[  "$images_exists" != "" ]]; then
        for image in $images_exists; do
            img_basename=$(basename "$image")
            simg2img "$image" "images/$build_filename/$img_basename"
        done
        cd images/$build_filename/
        7z a $build_filename.zip system.img vendor.img
        sha256sum $build_filename.zip > $build_filename.zip.sha256
        cd ../..
    fi
...
doGenerateChangelog() {
	...
	mv Changelog.txt images/$BASS_BUILD_FILENAME/Changelog-$BASS_BUILD_FILENAME.txt
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -f|--fossapps)
            ...
            shift
            ;;
        -b|--blissbuildvariant|--buildvariant)
            BLISS_BUILD_VARIANT="$2"
            # Confert $2 to all caps
            BASS_BUILD_VARIANT="${BLISS_BUILD_VARIANT}"
            LINEAGE_BUILDTYPE="${BLISS_BUILD_VARIANT}"
            shift
            shift
            ;;
        -i|--isgo)
...
export BASS_BUILD_VARIANT=${BASS_BUILD_VARIANT:-vanilla};
export LINEAGE_BUILDTYPE=${LINEAGE_BUILDTYPE:-vanilla};
...
export BASS_BUILD_FILENAME="${BASS_BUILD_PRE}${BASS_BUILD_POST}-${BASS_VERSION}_waydroid_${BASS_ARCH}_${BASS_BUILD_DATE}"
...
echo "DontuseBassinitinbuild: ${DONT_USE_BASS_INIT}";
echo "UseVaporLauncherasdefaulthome: ${USE_VAPOR_LAUNCHER}";
echo "Setdarkthemeasdefaultonbuild: ${SET_DARKMODE}";
echo "Defaultbuildarcharmarm64x86x8664: ${BASS_ARCH}";
...
echo -e "\033[1;34mLunch device: lineage_waydroid_${BASS_ARCH}-userdebug\033[0m"
breakfast lineage_waydroid_${BASS_ARCH}-userdebug 
lunch lineage_waydroid_${BASS_ARCH}-userdebug 
