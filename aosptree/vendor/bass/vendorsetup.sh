...
function copy_configs()
{
    ...
    if [[ "$USE_BLISS_RESTRICTED_LAUNCHER_PRO" = "true" ]] || [[ "$USE_BLISS_RESTRICTED_LAUNCHER" = "true" ]]; then
        ...
        echo ""
        { cat "$calamares_default_options"; cat "$aaropa_configs"/restricted/$calamares_presets_dir/restricted.options.yaml; } >$iso_dir/overlay/etc/$calamares_presets_dir/restricted.options.yaml
        cp -f "$aaropa_configs"/restricted/$calamares_presets_dir/icon/restricted.svg $iso_dir/overlay/etc/$calamares_presets_dir/icon/restricted.svg
        cat "$aaropa_configs"/restricted/grub/entry.cfg >>$iso_dir/boot/grub/entry.cfg
...
{
    # add "$GRUB_CMDLINE_OPTIONS" to the the specified grub config files, replacing the word "CMDLINE":
    if [ "$GRUB_CMDLINE_OPTIONS" != "" ]; then
        sed -i "s/CMDLINE/$GRUB_CMDLINE_OPTIONS/g" bootable/aaropa/iso/boot/grub/grub.cfg
    fi
}
