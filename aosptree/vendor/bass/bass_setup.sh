...
function update_apps()
{
    if [ "$UPDATE_FOSSAPPS" = "true" ]; then
        if [ "$USE_FOSSAPPS" = "true" ]; then
            ...
            echo ""
            $(cd vendor/foss && yes A | bash update.sh 1)
...
function validity_checks()
{
    ...
    if [ -f vendor/bass/prebuilts/bootsight/prebuilts/unsigned-priv-api-apps/BootSight.apk ]; then
        if [ ! $(sha256sum vendor/bass/prebuilts/bootsight/prebuilts/unsigned-priv-api-apps/BootSight.apk | grep -c "d9479d1adde5bf4f1842fe6828cd7e8d9306262ccdc6f94c6d0950438425e437") == 1 ]; then
...
function enroll_aaropa_configs()
{
	local aaropa_configs aaropa_default calamares_presets_dir calamares_presets_conf calamares_default_options iso_dir

	aaropa_configs=vendor/${vendor_name}/configs/aaropa_configs
	aaropa_default=vendor/${vendor_name}/aaropa
	calamares_presets_dir=calamares/resources/modules/presets
	calamares_presets_conf=${aaropa_default}/calamares/modules/presets.conf
	calamares_default_options=${aaropa_default}/${calamares_presets_dir}/default.options.yaml
	iso_dir=bootable/aaropa/iso

	local variant variant_calamares_dir variant_metadata_file variant_options_file variant_icon_file variant_grub_entry

	variant=$1
	variant_calamares_dir=${aaropa_configs}/${variant}/${calamares_presets_dir}
	variant_metadata_file=${variant_calamares_dir}/${variant}.metadata.yaml
	variant_options_file=${variant_calamares_dir}/${variant}.options.yaml
	variant_icon_file=${variant_calamares_dir}/icon/${variant}.svg
	variant_grub_entry=${aaropa_configs}/${variant}/grub/entry.cfg

	local target_calamares_dir target_calamares_options_file target_calamares_options_icon target_grub_entry

	target_calamares_dir=${iso_dir}/overlay/etc/${calamares_presets_dir}
	target_calamares_options_file=${target_calamares_dir}/${variant}.options.yaml
	target_calamares_options_icon=${target_calamares_dir}/icon/${variant}.svg
	target_grub_entry=${iso_dir}/boot/grub/entry.cfg

	# Copy default options and variant options
	cp -f "${calamares_default_options}" "${target_calamares_options_file}"
	cat "${variant_options_file}" >>"${target_calamares_options_file}"

	# Copy icon
    cp -f "${variant_icon_file}" "${target_calamares_options_icon}"

	# Add entry to presets.conf
	local meta_name meta_desc
	meta_name=$(grep name "${variant_metadata_file}" | awk -F ': ' '{print $2}')
	meta_desc=$(grep description "${variant_metadata_file}" | awk -F ': ' '{print $2}')
	cat <<EOF >>"${calamares_presets_conf}"
  - label: "${meta_name}"
	description: "${meta_desc}"
	icon: "/etc/${calamares_presets_dir}/icon/${variant}.svg"
	target: "file:///etc/${calamares_presets_dir}/${variant}.options.yaml"
EOF

	# Copy grub entry
	cat "${variant_grub_entry}" >>"${target_grub_entry}"
}
...
function copy_configs()
{
    ...
    if [ "$USE_TITANIUS_LAUNCHER" = "true" ]; then
        echo -e "Game-Mode launcher selected. Copying configs now..."
        echo ""
        enroll_aaropa_configs game_mode
        # cp -r vendor/$vendor_name/configs/config_defaults/game_mode/overlay/* vendor/$vendor_name/overlay/
        cp -r vendor/$vendor_name/configs/config_defaults/game_mode/dgc/* device/generic/common/
        sed -i 's/config_navBarInteractionMode">1/config_navBarInteractionMode">0/g' device/generic/common/overlay/frameworks/base/core/res/res/values/config.xml
        sed -i 's/config_navBarInteractionMode">2/config_navBarInteractionMode">0/g' device/generic/common/overlay/frameworks/base/core/res/res/values/config.xml
    ...
    if [ "$USE_BLISS_CROSS_LAUNCHER" = "true" ]; then
        ...
        echo ""
        enroll_aaropa_configs crosslauncher
    ...
    if [ "$BLISS_SECURE_LOCKDOWN_BUILD" = "true" ]; then
        ...
        echo ""
        enroll_aaropa_configs lockdown
        # cp -r vendor/$vendor_name/configs/config_defaults/kiosk/overlay/* vendor/$vendor_name/overlay/
        cp -r vendor/$vendor_name/configs/config_defaults/kiosk/dgc/* device/generic/common/
    ...
    if [ "$BLISS_LAUNCHER3_TASKBAR_NAVIGATION" = "true" ]; then
        ...
        sed -i 's/android:key="enable_taskbar" android:defaultValue="false"/android:key="enable_taskbar" android:defaultValue="true"/' packages/apps/Settings/res/xml/button_settings.xml || sed -i 's/android:key="enable_taskbar"/android:key="enable_taskbar" android:defaultValue="true"/' packages/apps/Settings/res/xml/button_settings.xml
    else
        sed -i 's/"ENABLE_TASKBAR", true,/"ENABLE_TASKBAR", false,/' packages/apps/Launcher3/src/com/android/launcher3/config/FeatureFlags.java
        sed -i 's/android:key="enable_taskbar" android:defaultValue="true"/android:key="enable_taskbar" android:defaultValue="false"/' packages/apps/Settings/res/xml/button_settings.xml || sed -i 's/android:key="enable_taskbar"/android:key="enable_taskbar" android:defaultValue="false"/' packages/apps/Settings/res/xml/button_settings.xml
        sed -i 's/android:key="enable_taskbar" android:defaultValue="true"/android:key="enable_taskbar" android:defaultValue="false"/' packages/apps/Settings/Blissify/res/xml/blissify_button.xml || sed -i 's/android:key="enable_taskbar"/android:key="enable_taskbar" android:defaultValue="false"/' packages/apps/Settings/Blissify/res/xml/blissify_button.xml
    fi
    ...
    if [ "$USE_AX86_STARTMENU" = "true" ]; then
        ...
        sed -i 's/"ENABLE_TASKBAR", false,/"ENABLE_TASKBAR", true,/' packages/apps/Launcher3/src/com/android/launcher3/config/FeatureFlags.java
        sed -i 's/android:key="enable_taskbar" android:defaultValue="false"/android:key="enable_taskbar" android:defaultValue="true"/' packages/apps/Settings/res/xml/button_settings.xml || sed -i 's/android:key="enable_taskbar"/android:key="enable_taskbar" android:defaultValue="true"/' packages/apps/Settings/res/xml/button_settings.xml
        sed -i 's/android:key="enable_taskbar" android:defaultValue="false"/android:key="enable_taskbar" android:defaultValue="true"/' packages/apps/Settings/Blissify/res/xml/blissify_button.xml || sed -i 's/android:key="enable_taskbar"/android:key="enable_taskbar" android:defaultValue="true"/' packages/apps/Settings/Blissify/res/xml/blissify_button.xml
...
\n\t<!-- Startmenu Button -->\n\t<resolve\n\t\tlauncher:screen="0"\n\t\tlauncher:x="0"\n\t\tlauncher:y="-1" >\n\t\tlauncher:packageName="com.ax86.startmenu"\n\t\tlauncher:className="com.ax86.startmenu.MainActivity"\n\t</resolve>\n' $file
    ...
    if [ "$BLISS_REMOVE_KSU" = "true" ]; then
        ...
        sed -i 's/CONFIG_KSU=y/\# CONFIG_KSU is not set/' kernel/x86/arch/x86/configs/android-x86_64_defconfig
        ...
        sed -i 's/\# CONFIG_KSU is not set/CONFIG_KSU=y/' kernel/x86/arch/x86/configs/android-x86_64_defconfig
	...
	if [ -f "$aaropa_configs/custom/$calamares_presets_dir/custom.options.yaml" ]; then
		...
		if [ ! -f "$aaropa_configs/custom/$calamares_presets_dir/icon/custom.svg" ]; then
			...
			cat <<EOF >>"$calamares_presets_conf"
  - label: "Custom"
    description: "User-defined variant"
	icon: "/etc/$calamares_presets_dir/icon/custom.svg"
	options: "/etc/$calamares_presets_dir/custom.options.yaml"
EOF
	...
	# Copy preset.conf
	cp -f "$calamares_presets_conf" $iso_dir/overlay/etc/calamares/modules/presets.conf
